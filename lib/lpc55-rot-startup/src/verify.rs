...
extern "C" {
    ...
    // static __IMAGE_A_BASE: [u32; ((__IMAGE_A_END as u32)-(__IMAGE_A_BASE as u32))/core::mem::size_of<u32>];
    static __IMAGE_A_BASE: *const u32;
    static __IMAGE_B_BASE: *const u32;
    static __IMAGE_BOOT_BASE: *const u32;
    static __IMAGE_STAGE_BASE: *const u32;
    static __IMAGE_A_END: *const u32;
    static __IMAGE_B_END: *const u32;
    static __IMAGE_BOOT_END: *const u32;
    static __IMAGE_STAGE_END: *const u32;
}

macro_rules! extern_u32_slice {
    ($start:expr, $end:expr) => {
...
pub fn verify_image(
    ...
    let (image, exec_image) = match which {
        SlotId::A => (
            extern_u32_slice!(__IMAGE_A_BASE, __IMAGE_A_END),
            extern_u32_slice!(__IMAGE_A_BASE, __IMAGE_A_END),
        ),
        SlotId::B => (
            extern_u32_slice!(__IMAGE_B_BASE, __IMAGE_B_END),
            extern_u32_slice!(__IMAGE_B_BASE, __IMAGE_B_END),
        ),
        SlotId::Bootloader => (
            extern_u32_slice!(__IMAGE_BOOT_BASE, __IMAGE_BOOT_END),
            extern_u32_slice!(__IMAGE_BOOT_BASE, __IMAGE_BOOT_END),
        ),
        // Safety: same
        SlotId::StagedBootloader => (
            extern_u32_slice!(__IMAGE_STAGE_BASE, __IMAGE_STAGE_END),
            extern_u32_slice!(__IMAGE_BOOT_BASE, __IMAGE_BOOT_END),
        ),
    };
    ...
    // shall we?
    {
        match header.image_type {
            ...
            // TODO: When using this from Hubris, we should CFPA at runtime.
            // If policy is being changed (CFPA allow-unsigned is being turned
            // off) then this gets more complicated.
        ...
        let image_addr_range = if bit_28_set {
            exec_image.as_ptr_range()
        } else {
            let r = exec_image.as_ptr_range();
        ...
        if !image_addr_range
            .contains(&((header.reset_vector & !1) as *const u32))
        {
    ...
    if lpc55_romapi::auth(image.as_ptr()) {
        Some((header, image))
    } else {
        None
    }
